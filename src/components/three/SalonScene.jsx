    ...
    <group ref={group} position={[0,.05,0]}>
      ...
      <group position={[-.38,-.12,.05]} rotation={[0,-.16,0]}>
        <mesh scale={[.72,1.45,.55]}>
          ...
          <meshPhysicalMaterial color="#7e5d45" metalness={.12} roughness={.18} clearcoat={.9} clearcoatRoughness={.1}/>
        </mesh>
        <mesh position={[0,.93,0]} scale={[.4,.38,.38]}>
          <cylinderGeometry args={[.55,.62,1,28]}/>
          <meshStandardMaterial color="#d6b78d" metalness={.9} roughness={.16}/>
        </mesh>
        <mesh position={[0,1.26,0]} scale={[.26,.22,.26]}>
          <cylinderGeometry args={[.54,.54,1,28]}/>
      ...
      <group position={[.78,-.48,.16]} rotation={[0,.28,0]}>
        <mesh scale={[.5,.9,.5]}>
          <cylinderGeometry args={[.72,.8,1.8,36]}/>
          <meshPhysicalMaterial color="#d9c4aa" metalness={.08} roughness={.24} clearcoat={.8}/>
        </mesh>
        <mesh position={[0,.9,0]} scale={[.34,.26,.34]}>
          <cylinderGeometry args={[.7,.7,1,28]}/>
          <meshStandardMaterial color="#8f6a4d" metalness={.72} roughness={.18}/>
        </mesh>
      </group>

      <mesh ref={halo} rotation={[1.22,.15,.42]} scale={2.25}>
        <torusGeometry args={[.78,.012,8,96]}/>
...
export default function SalonScene() {
  ...
  useEffect(() => {
    ...
    if (!node) return
    const observer = new IntersectionObserver(([entry]) => setActive(entry.isIntersecting), { threshold:.08 })
    observer.observe(node)
    return () => observer.disconnect()
  }, [])

  return (
    <div ref={wrap} className="salon-scene-wrap">
      <Canvas
        dpr={[1,1.15]}
